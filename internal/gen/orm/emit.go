...
func newFileEmit(p EmitParams) (*fileEmit, error) {
	f := &fileEmit{
		p:           p,
		seenImports: map[string]bool{},
...
// always-written imports pass through untouched; "time" folds into the
// conditional stdlib line the timestamps already use.
func (f *fileEmit) addTypeDeps(deps []PkgRef) {
	for _, d := range deps {
		switch d.Path {
		case f.p.SourcePath, runtimeImport, scopeImport:
			continue
		case "time":
			f.needsTime = true
			continue
		}
		if f.seenImports[d.Path] {
			continue
		}
		if isStdlib(d.Path) {
...
// its first segment carries no dot.
...
func (f *fileEmit) writeImports(b *strings.Builder) {
	...
	slices.Sort(f.stdlibImports)
	for _, path := range f.stdlibImports {
		fmt.Fprintf(b, "\t%q\n", path)
	}
	if f.hasTimestamps || f.needsTime {
		b.WriteString("\t\"time\"\n")
	}
	b.WriteString("\n")
	fmt.Fprintf(b, "\t%q\n", runtimeImport)
