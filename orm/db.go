// Package orm is the runtime for code generated by kanna-orm: the Query
// builder that generated factory functions return, the MySQL and PostgreSQL
// dialects that shape its SQL, and the transaction, logging, and clock
// plumbing around it.
package orm
...
type Querier interface {
	...
	// Transaction runs fn atomically. On DB it opens a transaction, commits
	// when fn returns nil, and rolls back on error or panic. On Tx it joins
	// the transaction already in flight, so nested calls compose into one
	// atomic unit — an inner error rolls back the whole transaction, never
	// just the inner part.
...
func (db *DB) Transaction(ctx context.Context, fn func(q Querier) error) (err error) {
	tx, err := db.Begin(ctx)
	if err != nil {
		return err
	}
...
// Transaction joins the transaction already in flight: fn runs on the same
// Tx, and commit or rollback stays with whoever opened it.
func (tx *Tx) Transaction(_ context.Context, fn func(q Querier) error) error {
	return fn(tx)
}
